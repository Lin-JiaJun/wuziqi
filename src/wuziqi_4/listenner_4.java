...
public class listenner_4 extends MouseAdapter implements ActionListener {
	// pass the graphics
	private Graphics g;
	...
	public void setA(ArrayList<shape_4> black) {
		this.black = black;
	}

	public void setB(ArrayList<shape_4> white) {
		this.white = white;
	}
			...
			if (x1 <= 600 && y1 <= 600) {
				...
				if (p != 0) {
					...
					if (black.size() == white.size()) {
						...
						shape_4 ii = new shape_4(x1, y1, g.getColor());
						ii.draw(g);
						// 判断胜利条件
						judgment_4 k = new judgment_4(black, white);
						win = k.judgeblack(x1, y1);
						black.add(ii);
						// 人机
						if (choose == 1 && win == 0) {
							g.setColor(Color.white);
							AI_4 i = new AI_4(black, white);
							i.Integer();// 计算最高权值
							shape_4 ii1 = new shape_4(i.getx1(), i.gety1(),
									g.getColor());
							ii1.draw(g);
							white.add(ii1);
							judgment_4 k1 = new judgment_4(black, white);
	...
	//	System.out.println(A.getActionCommand());
		...
		if(p == "认输"){
			off = 0;win=1;
			if(black.size()==white.size()){
				JOptionPane.showMessageDialog(null, "黑棋认输，白棋胜利");}else{
					JOptionPane.showMessageDialog(null, "白棋认输，黑棋胜利");
				}
		}
	}
}
